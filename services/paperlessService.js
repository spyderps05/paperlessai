...
class PaperlessService {
  ...
  async getOwnUserID() {
    this.initialize();
    try {
      console.log(`[DEBUG] getOwnUserID called. PAPERLESS_USERNAME env var: '${process.env.PAPERLESS_USERNAME}'`);
      ...
      console.log(`[DEBUG] Paperless API /users/ response status: ${response.status}`);
      if (response.data && response.data.results) {
        console.log(`[DEBUG] Paperless API /users/ results:`, JSON.stringify(response.data.results));
      } else {
        console.log(`[DEBUG] Paperless API /users/ response has no results or data is missing.`);
      }

      if (response.data.results && response.data.results.length > 0) {
        const userInfo = response.data.results;
        //filter for username by process.env.PAPERLESS_USERNAME (case-insensitive)
        const envUsername = process.env.PAPERLESS_USERNAME ? process.env.PAPERLESS_USERNAME.toLowerCase() : '';
        const user = userInfo.find(user => user.username.toLowerCase() === envUsername);

        if (user) {
          console.log(`[DEBUG] Found own user ID: ${user.id}`);
          return user.id;
        } else {
          console.warn(`[DEBUG] User '${envUsername}' not found in /users/ response. Available users in response: ${userInfo.map(u => u.username).join(', ')}`);
        }
      }
      return null;
    } catch (error) {
