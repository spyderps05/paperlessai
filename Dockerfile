...
# Configure application port - aber der tatsächliche Port wird durch GIMU_DOCUSAI_PORT bestimmt
EXPOSE ${GIMU_DOCUSAI_PORT:-3000}

# Add health check with dynamic port
HEALTHCHECK --interval=30s --timeout=30s --start-period=5s --retries=3 \
    CMD curl -f http://localhost:${GIMU_DOCUSAI_PORT:-3000}/health || exit 1
